package com.javaAngularProject.tasks;

import com.javaAngularProject.tasks.domain.Task;
import com.javaAngularProject.tasks.service.TaskService;
...
public class TasksApplication {
	...
	CommandLineRunner runner(TaskService taskService) {
		return args -> {
			...
			taskService.save( new Task(7L, "Create the Spring Boot Application", LocalDate.now().plus(12, ChronoUnit.DAYS), false) );
			taskService.save( new Task(8L, "Create the H2 Console for the initial data", LocalDate.now().plus(13, ChronoUnit.DAYS), false) );
		};
	}
